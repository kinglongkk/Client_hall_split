...
cc.Class({
		...
		for (let i = 0; i < allSwitchGameData.length; i++) {
			...
			switch (action) {
				case 'showForm':
					...
					break;
				case 'openClub':
					app.NetManager().SendPack("player.CPlayerSignInterface",{'sign':2});
		...
		for (let i = 0; i < allSwitchGameData.length; i++) {
			...
			switch (action) {
				case 'OpenUIRecordAllResult':
					...
					if (gameType == this.ShareDefine.GameType_GZMJ || gameType == this.ShareDefine.GameType_XGCGMJ) {
	...
	OnPack_GetGameStatus: function (serverPack) {
		...
		if (isPlayingGame) {//游戏中
			let gameName = app.ShareDefine().GametTypeID2PinYin[this.gameType];
			if (!gameName) {
				console.error("OnPack_GetGameStatus Error: Invalid gameType", this.gameType);
				// 游戏类型无效，只能返回大厅
				app.SceneManager().LoadScene("mainScene");
				return;
			}

			let sceneName = gameName; // 假设子游戏场景名和场景同名
			let currentScene = cc.director.getScene();

			// 【关键】检查当前是否已经在目标子游戏场景
			if (currentScene && currentScene.name === sceneName) {
				console.log("断线重连：已在子游戏场景 " + sceneName + "，无需切换，仅刷新数据。");
				// 通知当前场景刷新数据，这里可以发送一个全局事件
				app.Client.OnEvent("OnGameReconnect", serverPack);
			}
			else {
				console.log("断线重连：当前不在子游戏场景，需要加载场景 " + sceneName);
				// 如果当前不���子游戏场景，说明是从大厅断线后重连进游戏，此时需要加载子游戏
				// 注意：这里我们不再走 OnEvent_LoginGetCurRoomID 的老路，而是直接进入子游戏
				this.EnterSubGame(gameName, null, roomID);
			}
		} else {//跳转大厅
	...
	//        try {
	//        	                console.error("OnNativeNotify eventType:%s, eventDataString:%s",eventType, eventDataString);
	...
	OnInitClientFinish: function (allTableDataDict, clientConfig) {
		...
		try {
			...
			if (this.clientState != this.ShareDefine.State_InitSuccess) {
				                console.error("OnInitClientFinish 初始化异常");
				return
			}

			if (!this.InitModel()) {
				                console.error("OnInitClientFinish InitModel fail");
				return
			}

			if (!this.InitTable(allTableDataDict)) {
				                console.error("OnInitClientFinish InitTable fail");
				return
			}

			//初始化客户端所有管理器
			if (!this.InitManager()) {
				                console.error("OnInitClientFinish InitManager fail");
				return
			}
	...
	RefreshClientConfig: function () {
		console.log("ClientManageer this.clientConfig:", JSON.stringify(this.clientConfig));
		...
		this.SysLog("ClientManageer this.clientConfig2:", this.clientConfig);
	...
	InitManager: function () {
		...
		try {
			for (let index = 0; index < count; index++) {
				console.log("InitManager", modelName)
				modelName = NeedCreateList[index];
	...
	OnEvent: function (eventName, argDict) {
		...
		try {
			...
			this.node.emit(eventName, argDict);
			                ...
			                console.error("大厅事件分发 OnEvent:%s", error.stack)
		}
	},
		...
		if (this.clientConfig.hasOwnProperty(property)) {
			if (mt.InArray(property)) {
				...
				if (property != "WeChatAppShareUrl") {
					...
					if (this.GetAppName() == "xinhua") {
						return this.clientConfig["WeChatAppShareUrl"];
					} else if (this.GetAppName() == "baodao") {
						return this.clientConfig["WeChatAppShareUrlBaoDao"];
					}else if (this.GetAppName() == "sansan") {
						return this.clientConfig["WeChatAppShareUrlSanSan"];
					}else if (this.GetAppName() == "ganzhou") {
						return this.clientConfig["WeChatAppShareUrlGanZhou"];
					}else if (this.GetAppName() == "jiangxi") {
						return this.clientConfig["WeChatAppShareUrlJiangXi"];
					}else if (this.GetAppName() == "hubei") {
						return this.clientConfig["WeChatAppShareUrlHuBei"];
					}
				}
			}
		} else {
	...
	StartEnterGame: function (name, switchGameData = null) {
		...
		app.NetManager().SendPack("room.CBaseGameTypeUrl", {"gametype": gamtTypeID}, function (success) {
			...
			app.NetManager().SendPack("base.C1110UUID", {"gameName": name}, function (event) {
			    ...
			    app.SoundManager().StopAllSound();
			    //如果是进游戏切换需要释放内存，重新加载
				that.RemoveClientManager();
			    //提前设置游戏的标识符，防止进游戏后重连导致继续发送大厅的游戏
				cc.sys.localStorage.setItem("curRunGame", name);
	...
	JoinRoomCheckSubGame: function (gameName, roomKey, clubId = 0, haveEnterFunction = undefined, password = "", existQuickJoin = false) {
		let mainComponent = app.FormManager().GetFormComponentByFormName(app.FormManager().MainForm());
		...
		if (SubgameManager.isSubgameDownLoad(gameName)) {
			...
			SubgameManager.needUpdateSubgame(gameName, (success) => {
				if (success) {
					...
					if (sendPack.clubId && sendPack.clubId != 0) {
						app.NetManager().SendPack("club.CClubCreateRoom", sendPack, function (event) {
							console.log("创建房间成功", event, JSON.stringify(event));
						}, function (error) {
							console.log("亲友圈创建房间失败", error, JSON.stringify(error));
						});
					} else {
						console.log("[建房请求] sendPack =", sendPack);

						app.NetManager().SendPack("room.CBaseCreateRoom", sendPack, function (event) {
							console.log("创建房间成功", event, JSON.stringify(event));
						}, function (error) {
							console.log("创建房间失败", error, JSON.stringify(error));
							app.SysNotifyManager().ShowSysMsg("创建房间失败", [], 3);
						});
					}
				}
	...
	GetAllGameId: function (isGetAllGame = false) {
		...
		if (this.allGameIdFormServer.length==0) {
			//如果服务端下发游戏列表未取到
			this.GetAllGameIdFromServer(true);
			return [];
		}
		...
		console.log('allGameId:', JSON.stringify(allGameId), 'allGameIdFormtemp', JSON.stringify(allGameIdFormtemp));
		// let gameCount = 3;
		// if (allGameId.length < gameCount) {
		//     for (let j = 0; j < allGameId.length; j++) {
		//         let index = allGameIdFormtemp.indexOf(allGameId[j]);
		//         if (index > -1) {
		//             allGameIdFormtemp.splice(index, 1);
		//         }
		//     }
		//     let needAddCount = gameCount - allGameId.length;
		//     for (let i = 0; i < needAddCount; i++) {
		//         //如果没有3个游戏，把未下载的加入
		//         if (allGameIdFormtemp.length <= 0) {
		//             break;
		//         }
		//         allGameId.push(allGameIdFormtemp[0]);
		//         allGameIdFormtemp.splice(0, 1);
