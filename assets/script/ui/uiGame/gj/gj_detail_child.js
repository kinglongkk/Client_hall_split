...
cc.Class({
	...
	ShowPlayerData: function (resultsList, playerAll, idx) {
        ...
        //显示是否投降（-1:正常结束,0:未操作,1:同意操作,2:拒绝操作,3:发起者）
        if (typeof(data.surrenderState) == "undefined" || data.surrenderState != 3) {
            this.node.getChildByName('icon_surrender').active=false;
        }else{
            let imagePath = "texture/record/img_surrender"+data.surrenderState;
            let that = this;
            //加载图片精灵
            cc.loader.loadRes(imagePath, cc.SpriteFrame, function (error, spriteFrame) {
                if(error){
                ...
                that.node.getChildByName('icon_surrender').getComponent(cc.Sprite).spriteFrame = spriteFrame;
                that.node.getChildByName('icon_surrender').active=true;
            });
        }
	},
});
