...
var BaseForm = cc.Class({
	...
	//force==true ，强制弹出
	ShowShiMing:function(){ //force==true,没在赛事，强制弹出
		if(app["haveClub"]==1){
			return; //有在赛事就不用被弹出
		}
        ...
        if(!realNumber || !realName){
            ...
            if(age<18){
            	app.SysNotifyManager().ShowSysMsg("您未满18周岁,当天紧能游戏60分钟", [], 3);
            }
        }
    },
    ShowShiMingOnce:function(){
	...
	SetWndImage: function(wndPath, imageName) {
		...
		this.SetWndImageByFilePath(wndPath, filePath, fileName)
		  .then(success => {
			if (!success) {
			  this.ErrLog(`SetWndImage failed for wndPath=${wndPath}, filePath=${filePath}, fileName=${fileName}`);
			}
		  })
		  .catch(err => {
			this.ErrLog(`SetWndImage error: ${err.stack || err}`);
		  });
	},
	...
	GetTuiGuangUrl:function(){
        ...
        if(this.TuiGuangUrl == ''){
        	...
        	let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://185.213.26.71:88/"+ShortHeroID+'/';
    ...
    SuccessTcnUrl:function(serverPack){
        let shareUrl="https://185.213.26.71:88/"+app.ComTool().GetPid(app.HeroManager().GetHeroProperty("pid"))+"/";
    	if(this.TuiGuangUrl){
        	text=this.ShareText+"\n"+"下载地址:"+shareUrl;
    	}else{
    		if(url_short){
    ...
    BeginShare:function(roomKey,text){
    	...
    	//校服
    	let long_url="http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=https://185.213.26.71:88/room"+roomKey;
