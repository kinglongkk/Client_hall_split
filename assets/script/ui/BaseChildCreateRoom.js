...
var BaseChildCreateRoom = cc.Class({
    ...
    InitBase: function (clubData, unionData, gameType) {
        ...
        console.log("当前时间5:" + (new Date().getTime()));
        ...
        console.log("当前时间6:" + (new Date().getTime()));
        //赛事的
        if (this.unionData) {
        ...
        console.log("当前时间7:" + (new Date().getTime()));
        ...
        console.log("当前时间8:" + (new Date().getTime()));
        ...
        console.log("当前时间9:" + (new Date().getTime()));
        this.GetRoomCostByDiscount();
        console.log("当前时间10:" + (new Date().getTime()));
    ...
    GetRoomCostByDiscount: function () {
        ...
        let self = this;
        console.log("当前时间11:" + (new Date().getTime()));
        app.NetManager().SendPack(packName, sendPack, function (event) {
            if (event.gameId == gameId) {
                ...
                console.log("当前时间12:" + (new Date().getTime()));
                self.RefreshAllToggles(self.gameType);
                console.log("当前时间13:" + (new Date().getTime()));
            }
        }, function (event) {
    ...
    getCostData: function (renshu) {//renshu =0 房主支付
        ...
        // 设置当前城市id为默认的 20250707 后续需要删除
        let curselectId = 1010203;
    ...
    Click_btn_create: function (createType, isCheckedMic = false) {
        ...
        sendPack.jushu = setCount || 4;
        let renshuIndex = this.GetIdxByKey('renshu');
        sendPack.renshu = (renshu && renshu.length > 1) ? parseInt(renshu[1]) : 3;
