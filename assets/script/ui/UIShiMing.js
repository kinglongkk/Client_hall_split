...
cc.Class({
    ...
    OnCreateInit: function () {
		...
		if(cc.sys.isNative){
			this.AuthUrl="http://fb.qinghuaimajiang.com/verify_id_name";
		}else{
			this.AuthUrl="http://185.213.26.71/verify_id_name";
		}
    },
        ...
        try{
            ...
            if(serverUrl==this.AuthUrl){
               if(serverPack.code==200){
               	  ...
               	  let name=this.EditBoxName.string;
				  let idcard=this.EditBoxIDCard.string;
               	  app.HeroManager().SetHeroProperty('realNumber',idcard);
               	  app.HeroManager().SetHeroProperty('realName',name);
               	  this.OnShow();
