...
var NetWork = app.BaseClass.extend({
			...
			if (allPack > 1) {
				...
				if (this.BigPack.length == allPack) {
					...
					this.BigPack = [];
					                ...
					                console.error("NetWork allPack error length:%s", allPack);
					return;
				}
			}
	...
	OnClose: function (event) {
		...
		if (this.url != event.target.URL) {
			return;
		                ...
		                console.error("OnClose:%s,%s", this.url, event.type);
		if (this.isReconnecting) {
	...
	SendLostPack: function () {
		if (app.Client.isInHall()==false) {
        	console.log("不在大厅内，SendLostPack");
        	return;
        }
	...
	write: function (head, message) {
		...
		//发送数据
		console.log("开始发送封包数据：" + head.event);
